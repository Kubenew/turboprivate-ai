...
class BackupManager:
    ...
    async def create(self, name: str, encrypt: bool = False, passphrase: str | None = None) -> Path:
        ...
        if kubeconfig.exists():
            (backup_dir / "kubeconfig.yaml").write_text(kubeconfig.read_text())
        config = Path("turboprivate.yaml")
        if config.exists():
            (backup_dir / "turboprivate.yaml").write_text(config.read_text())
        try:
        ...
        if encrypt:
            encrypted_path = self.backup_path / f"{backup_name}.tar.gz.age"
            pw = passphrase or os.environ.get("TURBO_BACKUP_PASS", "")
            if pw:
                proc = await asyncio.create_subprocess_exec(
                    "age", "--passphrase", "--output", str(encrypted_path), str(tarball),
    ...
    async def restore(self, name: str, target: Path | None = None):
        ...
        if backup_file.suffix == ".age":
            ...
            proc = await asyncio.create_subprocess_exec(
                "age", "--decrypt", "--output", str(restore_dir / "backup.tar.gz"),
                str(backup_file), stdin=asyncio.subprocess.PIPE,
            )
            await proc.communicate(input=pw.encode())
        ...
        kubeconfig = restore_dir / Path(backup_file.stem).stem / "kubeconfig.yaml"
        if kubeconfig.exists():
            Path("kubeconfig.yaml").write_text(kubeconfig.read_text())
