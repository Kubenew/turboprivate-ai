...
@router.get("/models")
async def list_models(gateway: InferenceGateway = Depends(get_inference_gateway)):
...
async def pull_model(req: PullRequest):
    return {"status": "started", "model": req.name, "message": f"Pulling model {req.name} in background"}
...
async def quantize_model(req: QuantizeRequest):
    ...
    return {"status": "completed", "model": req.name, "bits": req.bits, "method": req.method, "output_path": output_path}
